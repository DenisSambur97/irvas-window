...
var forms = function forms() {
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      var formData = new FormData(item); // Получаем все данные с формы определённой
...
var tabs = function tabs(headerSelector, tabSelector, contentSelector, activeClass) {
  ...
  hideTabContent();
  showTabContent(); // Делегирование события хедеру для табов

  header.addEventListener('click', function (event) {
    var target = event.target; // Проверка на наличие таргета и что клик произошел на нашил табах

    if (target && (target.classList.contains(tabSelector.replace(/\./, '')) || target.parentNode.classList.contains(tabSelector.replace(/\./, '')))) {
      // Убираем точку из tabSelector'a
